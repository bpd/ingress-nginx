...
func (ic *GenericController) getBackendServers(ingresses []*extensions.Ingress) ([]*ingress.Backend, []*ingress.Server) {
	...
	// create a map of upstream.Name => upstream
	upstreamMap := make(map[string]*ingress.Backend)
	...
	for _, server := range servers {
		for _, location := range server.Locations {
			...
			// do we have an upstream for this location?
			upstream := upstreamMap[location.Backend]
			if upstream == nil {
				continue
			}
			// upstream.Name == location.Backend
